...
export function transformText(input: string, context: vscode.ExtensionContext, sidebarProvider: SidebarProvider): Promise<string> | null {
    ...
    return new Promise((resolve, reject) => {
        let reg = /(?<!(\/\/|\/\*\*).*)[a-zA-Z]*[\u4e00-\u9fa5（），]{2,}[a-zA-Z]*[\u4e00-\u9fa5（）]*[\:\：\？\?！]?/g;
        ...
        if (allzh.length !== 0) {
            getConfig(allzh).then(res => {
                ...
                resolve(JSON.stringify({ text: toReplace(input, res, moduleName), config: res }));
            });
        }
    });
}
    ...
    return new Promise((resolve, reject) => {
        ...
        translate(str).then((item) => {
            ...
            for (let i = 0; i < enArr.length; i++) {
                ...
                let resKey = arr.join('').replace(':', 'Col').replace(/\(.*\)/g, '').replace(/[,，！\!\-]/g, '');
...
function toReplace(str: string, obj: { [key: string]: string }, moduleName: string): string {
    Object.entries(obj).forEach(([key, value]) => {
        ...
        str = str.replaceAll(`label="${value}"`, `:label="$t('${moduleName}.${key}')"`)
...
function translate(str: string): Promise<string> {
    return new Promise((resolve, reject) => {
        ...
        let url = `http://api.fanyi.baidu.com/api/trans/vip/translate?q=${encodeURIComponent(q)}&from=zh&to=en&appid=${appid}&salt=${salt}&sign=${sign}`;
        request({
            url,
            method: 'GET'
